(function () {
   ...
   `;
    ...
    //
    const overlay = document.createElement('div');
    overlay.className = 'Xeno-overlay-broken';       // changed

    const XenoTitle = document.createElement('h3');
    XenoTitle.className = 'Xeno-title-broken';      // changed
    XenoTitle.innerText = `XENO 1.0`;
    ...
    document.querySelector('#ui-top-left').insertBefore(XenoTitle, document.querySelector('#ui-top-left').firstChild);
    ...
    // Ð¡Ð¾Ð·Ð´Ð°Ð½Ð¸Ðµ ÑÐ»ÐµÐ¼ÐµÐ½ÑÐ° Ñ Ð·Ð°Ð´Ð°Ð½Ð½ÑÐ¼Ð¸ ÑÑÐ¸Ð»ÑÐ¼Ð¸
    ...
    // Ð¡Ð¾Ð·Ð´Ð°Ð½Ð¸Ðµ ÐºÐ½Ð¾Ð¿ÐºÐ¸ ÑÑÐ½ÐºÑÐ¸Ð¸
    ...
    // Ð¡Ð¾Ð·Ð´Ð°Ð½Ð¸Ðµ ÐºÐ¾Ð½ÑÐµÐ¹Ð½ÐµÑÐ° UI
    ...
    // Ð¡Ð¾Ð·Ð´Ð°Ð½Ð¸Ðµ ÑÐ¾Ð´ÐµÑÐ¶Ð¸Ð¼Ð¾Ð³Ð¾
    ...
    // Ð¡Ð¾Ð·Ð´Ð°ÐµÐ¼ Ð½Ð°Ð±Ð»ÑÐ´Ð°ÑÐµÐ»Ñ Ð·Ð° Ð¸Ð·Ð¼ÐµÐ½ÐµÐ½Ð¸ÑÐ¼Ð¸ Ð°ÑÑÐ¸Ð±ÑÑÐ¾Ð²
    const observer = new MutationObserver(syncMenuVisibility);

    // ÐÐ°ÑÐ¸Ð½Ð°ÐµÐ¼ Ð½Ð°Ð±Ð»ÑÐ´ÐµÐ½Ð¸Ðµ Ð·Ð° Ð¸Ð·Ð¼ÐµÐ½ÐµÐ½Ð¸ÑÐ¼Ð¸ Ð°ÑÑÐ¸Ð±ÑÑÐ¾Ð² Ñ ÑÐ»ÐµÐ¼ÐµÐ½ÑÐ° #ui-game-menu
    const gameMenu = document.getElementById('ui-game-menu');
    if (gameMenu) {
        observer.observe(gameMenu, { attributes: true, attributeFilter: ['style'] });
    }

    // ÐÐ¾Ð±Ð°Ð²Ð»ÐµÐ½Ð¸Ðµ ÑÑÐ¸Ð»ÐµÐ¹ Ð°Ð½Ð¸Ð¼Ð°ÑÐ¸Ð¸
...
@keyframes fadeIn {
    ...
    class GameMod {
        ...
        initMenu() {
            ...
            description.innerHTML = `â­ Star us on GitHub<br>ð¢ Join our Telegram group<br>ð® Join our Discord server`;
            ...
            discordLink.href = "https://discord.gg/NCGUAmDM2p"; // Updated link
...
<h2>Welcome to Xenos free Cheatsð</h2>
...
<p class="news-paragraph">For more details, visit the <a href="https://github.com/Drino955/survev-kr1tyhack">GitHub page</a> and join our <a href="https://t.me/kr1tyteam">Telegram group</a> or <a href="https://discord.gg/NCGUAmDM2p">Discord</a>.</p></div>`;
    ...
    const playerNameInput = document.getElementById('player-name-input-solo');
    if (playerNameInput) {
        playerNameInput.value = 'discord.gg/krity';
...
`;
  ...
  function autoSwitch() {
    ...
    try {
        ...
        console.log("Current Weapon Index:", curWeapIdx, "Current Weapon:", curWeap);

        const shouldSwitch = (gun) => {
            let s = false;
            try {
                s = (unsafeWindow.guns[gun]?.fireMode === "single" || unsafeWindow.guns[gun]?.fireMode === "burst") && unsafeWindow.guns[gun]?.fireDelay >= 0.45;
            } catch (e) {
        ...
        if (curWeap.ammo !== ammo[curWeapIdx]?.ammo) {
            ...
            if (
                ...
                (unsafeWindow.game.touch.shotDetected || unsafeWindow.game.inputBinds.isBindDown(inputCommands.Fire)))) &&
   ...
   function grenadeTimer() {
    ...
    function initTicker() {
        ...
        unsafeWindow.game.pixi._ticker.add(grenadeTimer); // Ensure this is included
