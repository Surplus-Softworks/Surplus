...
function drawLasers(me, players, laserDrawer) {
    ...
    function laserPointer(acPlayer, curBullet, curWeapon, color = 0x0000ff, opacity = 0.1) {
        ...
        const atan = acPlayer === me && !lastAimPos 
            ? Math.atan2(gameManager.game.input.mousePos.y - innerHeight / 2, gameManager.game.input.mousePos.x - innerWidth / 2)
    ...
    players.filter(player => player.active && !player.netData.dead && me.__id !== player.__id && me.layer === player.layer && getTeam(player) !== getTeam(me))
        .forEach(enemy => {
